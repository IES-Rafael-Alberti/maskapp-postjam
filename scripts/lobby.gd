...
func player_loaded():
	if multiplayer.is_server():
		debug_log("player loaded %d" % multiplayer.get_remote_sender_id())
		players_loaded += 1
...
func _on_peer_connected(id):
	if multiplayer.is_server() or not client_connecting:
		debug_log("player connected %d" % id)
		_register_player.rpc_id(id, player_info)

@rpc("any_peer", "reliable")
func _register_player(new_player_info):
	var new_player_id = multiplayer.get_remote_sender_id()
	debug_log("player registered %d" % new_player_id)
...
func _on_connected_ok():
	...
	debug_log("player connected %d ok" % peer_id)
